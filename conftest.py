...
#сохранять статус тест кейса. если упал, то СМ НИЖЕ
...
def web_browser(request, selenium):
    ...
    yield browser
    #После йэлда, выполняется код из теста. после заверщения теста, всё что ниже
    # если тест упал, то сделать логи ниже. если нет, то ничего не делать

    # Do teardown (this code will be executed after each test):
    if request.node.rep_call.failed:
        # Make the screen-shot if test failed:
        try:
            # Если упал, то подкладывать белый фон
            browser.execute_script("document.body.bgColor = 'white';")

            # Make screen-shot for local debug:
            browser.save_screenshot('screenshots/' + str(uuid.uuid4()) + '.png')

            # В аллюр сркиншот добавить
            # Attach screenshot to Allure report:
            # allure.attach(browser.get_screenshot_as_png(),
            #               name=request.function.__name__,
            #               attachment_type=allure.attachment_type.PNG)

            # Писать в консоль лог браузера
            # For happy debugging:
